...
class CyberGear:
    def __init__(self, com_port="COM3", baud_rate=115200, model="CAN") -> None:
        ...
        Args:
            ...
            model: USB to CAN module model (default is "CAN")
    ...
    def _can_to_uart(self, data=[], rtr=0):
    ...
    def _send_can(self, id_num=127, cmd_mode=0, cmd_data=[], data=[], rtr=0):
        """CAN send function.
        ...
        for i in range(8):
            cdata[5 + i] = data[i]
        self._write_port(data=self._can_to_uart(data=cdata, rtr=rtr))
    ...
    def _format_data(self, data=[], format="f f", type="decode"):
    ...
    def _reply_state(self, id_num=127):
        ...
        try:
            if id_num <= self.MOTOR_NUM:
                ...
                if self.READ_FLAG == 1 and rdata[1] == 2:
                    ...
                    id_num = rdata[3]
                    data = rdata[5:]
                    self.motor_state[id_num - 1][0] = (
                        self._uint_to_float(
                            (data[0] << 8) + data[1], self.P_MIN, self.P_MAX, 16
                        )
                        * self.RAD_DEG
                    )
                    self.motor_state[id_num - 1][1] = (
                        self._uint_to_float(
                            (data[2] << 8) + data[3], self.V_MIN, self.V_MAX, 16
                        )
                        * self.RAD_S_R_MIN
                    )
                    self.motor_state[id_num - 1][2] = self._uint_to_float(
                        (data[4] << 8) + data[5], self.T_MIN, self.T_MAX, 16
                    )
    ...
    def _write_prop(self, id_num=127, index=0, data_type="f", value=0):
        ...
        tx_data[4:] = self._format_data(data=[value], format=data_type, type="encode")
    ...
    def _read_prop(self, id_num=127, index=0, data_type="f"):
    ...
    def motor_enable(self, id_num=127):
        ...
        self._send_can(
            id_num=id_num, cmd_mode=3, cmd_data=cmd_data, data=tx_data, rtr=0
        )
        self._reply_state(id_num=id_num)

    def motor_stop(self, id_num=127):
        ...
        self._send_can(
            id_num=id_num, cmd_mode=4, cmd_data=cmd_data, data=tx_data, rtr=0
        )
        self._reply_state(id_num=id_num)

    def set_mode(self, id_num=127, mode=0):
        ...
        self._write_prop(id_num=id_num, index=0x7005, value=mode, data_type="u8")

    def impedance_control(self, id_num=127, pos=0, vel=0, tff=0, kp=0, kd=0):
        ...
        try:
            ...
            cmd_data = [0] * 2
            cmd_data[0] = (self._float_to_uint(tff, self.T_MIN, self.T_MAX, 16)) & 0xFF
            cmd_data[1] = (
                (self._float_to_uint(tff, self.T_MIN, self.T_MAX, 16)) >> 8
            ) & 0xFF
            tx_data = [0] * 8
            tx_data[0] = (
                self._float_to_uint(pos * self.DEG_RAD, self.P_MIN, self.P_MAX, 16) >> 8
            ) & 0xFF
            tx_data[1] = (
                self._float_to_uint(pos * self.DEG_RAD, self.P_MIN, self.P_MAX, 16)
            ) & 0xFF
            tx_data[2] = (
                self._float_to_uint(vel * self.R_MIN_RAD_S, self.V_MIN, self.V_MAX, 16)
                >> 8
            ) & 0xFF
            tx_data[3] = (
                self._float_to_uint(vel * self.R_MIN_RAD_S, self.V_MIN, self.V_MAX, 16)
            ...
            tx_data[4] = (
                self._float_to_uint(kp, self.KP_MIN, self.KP_MAX, 16) >> 8
            ) & 0xFF
            tx_data[5] = (self._float_to_uint(kp, self.KP_MIN, self.KP_MAX, 16)) & 0xFF
            tx_data[6] = (
                self._float_to_uint(kd, self.KD_MIN, self.KD_MAX, 16) >> 8
            ) & 0xFF
            tx_data[7] = (self._float_to_uint(kd, self.KD_MIN, self.KD_MAX, 16)) & 0xFF
            # Need to send using extended frame (data frame)
            self._send_can(
                id_num=id_num, cmd_mode=1, cmd_data=cmd_data, data=tx_data, rtr=0
            )
            self._reply_state(id_num=id_num)
        except Exception as e:
            print("!!!ERROR IN IMPENDENCE CONTROL:", e)

    def set_pos(self, id_num=127, pos=0, vel=10, limit_cur=27):
        ...
        self.motor_enable(id_num=id_num)
        self.set_mode(id_num=id_num, mode=1)
        self._write_prop(id_num=id_num, index=0x7018, value=limit_cur, data_type="f")
        self._write_prop(
            id_num=id_num, index=0x7017, value=vel * self.R_MIN_RAD_S, data_type="f"
        )
        self._write_prop(
            id_num=id_num, index=0x7016, value=pos * self.DEG_RAD, data_type="f"
        )

    def set_vel(self, id_num=127, vel=10, limit_cur=27):
        ...
        self.motor_enable(id_num=id_num)
        self.set_mode(id_num=id_num, mode=2)
        self._write_prop(id_num=id_num, index=0x7018, value=limit_cur, data_type="f")
        self._write_prop(
            id_num=id_num, index=0x700A, value=vel * self.R_MIN_RAD_S, data_type="f"
        )

    def set_torque(self, id_num=127, torque=0.1):
        ...
        self.motor_enable(id_num=id_num)
        self.set_mode(id_num=id_num, mode=3)
        self._write_prop(
            id_num=id_num,
            index=0x7006,
    ...
    def set_zero(self, id_num=127):
        ...
        # Need to send using extended frame (data frame)
        self._send_can(
            id_num=id_num, cmd_mode=6, cmd_data=cmd_data, data=tx_data, rtr=0
        )
        self._reply_state(id_num=id_num)
        if mode_status == 2:
            self.motor_enable(id_num=id_num)

    def set_id(self, id_num=127, new_id=1):
        ...
        if len(MCU_ID) == 8:
            ...
            # Need to send using extended frame (data frame)
            self._send_can(
                id_num=id_num, cmd_mode=7, cmd_data=cmd_data, data=tx_data, rtr=0
            )
            time.sleep(0.1)
            self._reply_state(id_num=id_num)
            return True
    ...
    def clear_error(self, id_num=127):
    ...
    def restore_config(self, id_num=127):
        ...
        # Need to send using extended frame (data frame)
        self._send_can(
            id_num=id_num, cmd_mode=8, cmd_data=cmd_data, data=tx_data, rtr=0
        )
    ...
    def get_posvel(self, id_num=127):
        ...
        try:
            ...
            self._write_prop(id_num=id_num, index=0x7018, value=27, data_type="f")
    ...
    def get_volcur(self, id_num=127):
